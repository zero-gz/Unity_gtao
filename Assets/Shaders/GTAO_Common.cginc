...
half4 GTAO(half2 uv, int NumCircle, int NumSlice, inout half Depth)
{
	...
	for (int i = 0; i < NumCircle; i++)
	{
		...
		slideDir_TexelSize = sliceDir.xy * _AO_RT_TexelSize.xy; //xy是 屏幕尺寸的倒数，zw是屏幕尺寸，这里算出来是世界空间的长度在投影平面的uv长度
		h = -1;
		...
		for (int j = 0; j < NumSlice; j++)
		{
			...
			uvSlice = uv.xyxy + float4(uvOffset.xy, -uvOffset); //沿着旋转角度的方向，不断加减得到相应变换后的uv
